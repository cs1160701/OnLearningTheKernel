...
import torch
from torch.nn import init, Dropout, LayerNorm, Linear, Module, ModuleList
...
class TransformerEncoderLayer(Module):
    ...
    ---------
        ...
        axial_ordering: If set to true the Encoder layers are defined as in 
                        the Axial Transformer (https://arxiv.org/pdf/1912.12180.pdf).
                        Otherwise use conventional definition (default: False)
        event_dispatcher: str or EventDispatcher instance to be used by this
                          module for dispatching events (default: the default
                          global dispatcher)
    """
    def __init__(self, attention, d_model, d_ff=None, dropout=0.1,
                 activation="relu", bias=True, weight_init=None, 
                 bias_init=None, axial_ordering=False, event_dispatcher=""):
        super(TransformerEncoderLayer, self).__init__()
        d_ff = d_ff or 4*d_model
        self.attention = attention
        self.axial_ordering = axial_ordering
        self.linear1 = Linear(d_model, d_ff, bias=bias)
        self.linear2 = Linear(d_ff, d_model, bias=bias)
    ...
    def reset_parameters(self, bias, weight_initializer, bias_initializer): 
        ...
        if bias and bias_initializer == 'normal':
            init.normal_(self.linear1.bias, std=1e-6)
            init.normal_(self.linear2.bias, std=1e-6)
    ...
    def forward(self, x, attn_mask=None, length_mask=None):
        ...
        # Apply layer norm in axial setting 
        if self.axial_ordering: 
            x = self.norm2(x)

        # Run self attention and add it to the input
        s = x = x + self.dropout(self.attention(
            x, x, x,
            attn_mask=attn_mask,
...
class TransformerDecoderLayer(Module):
    ...
    def __init__(self, self_attention, cross_attention, d_model, d_ff=None,
                 dropout=0.1, activation="relu", bias=True, weight_init=None, 
